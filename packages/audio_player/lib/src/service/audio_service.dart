...
/// Configuration for audio service initialization
class AudioServiceConfig {
  ...
  const AudioServiceConfig({
    ...
    this.androidShowNotificationBadge = true,
  });
}
...
class AudioServiceManager {
  ...
  /// Initialize the audio service with optional configuration
  Future<void> initialize({AudioServiceConfig? config}) async {
    // If we already have a handler, don't reinitialize
    if (_handler != null && _isInitialized) return;
    
    _config = config ?? const AudioServiceConfig(
      androidNotificationChannelId: 'com.audio_player.audio',
    ...
    try {
      _handler = await audio_service.AudioService.init(
        ...
        config: audio_service.AudioServiceConfig(
          androidNotificationChannelId: _config!.androidNotificationChannelId,
          androidNotificationChannelName: _config!.androidNotificationChannelName,
          androidNotificationChannelDescription: _config!.androidNotificationChannelDescription,
          androidNotificationOngoing: _config!.androidNotificationOngoing,
          androidStopForegroundOnPause: _config!.androidStopForegroundOnPause,
          androidShowNotificationBadge: _config!.androidShowNotificationBadge,
          androidNotificationIcon: _config!.androidNotificationIcon,
          notificationColor: _config!.notificationColor,
        ),
      );
      _isInitialized = true;
